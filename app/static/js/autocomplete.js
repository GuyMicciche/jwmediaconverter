...
function fuzzyMatch(str, lookup) {
    const queryWords = lookup.split(' ').filter(Boolean);  // Split query into words and filter empty strings
    let textIdx = 0;

    for (let i = 0; i < queryWords.length; i++) {
        const queryWord = queryWords[i];
        const foundIdx = str.indexOf(queryWord, textIdx);
        if (foundIdx === -1) {
            return false;  // If any word in the query is not found in sequence, no match
        }
        textIdx = foundIdx + queryWord.length;
    }
    return true;  // All words in query were found in order
}


/**
 * A custom multi-word matching algorithm.
 * @param {String} str
 * @param {String} lookup
 * @returns {Boolean}
 */
function multiWordMatch(str, lookup) {
    const words = lookup.split(" ").filter(word => word.length > 0);
    return words.every(word => fuzzyMatch(str, word));
}
...
class Autocomplete {
    ...
    onblur(e) {
        ...
        // In chrome >= 127, the related
        // target is the dropdown menu
    ...
    _createItem(lookup, item) {
        ...
        const queryWords = lookup.split(' ').filter(Boolean);  // Split the lookup into words

        if (this._config.highlightTyped) {
            const highlightWords = (label, queryWords) => {
                let regexStr = queryWords.map(word => `(${word})`).join('|');
                const regex = new RegExp(regexStr, 'gi');

                // This function will highlight each word without causing overlap
                return label.replace(regex, (match) => {
                    return `<mark class="${this._config.highlightClass}">${match}</mark>`;
                });
            };

            label = highlightWords(label, queryWords);  // Apply the highlighting in a single pass
        }

        label = this._config.onRenderItem(item, label, this);  // Apply any custom rendering logic
        ...
        newChildLink.innerHTML = label;  // Use innerHTML to ensure the <mark> tags are rendered properly
        ...
        // Apply value when clicked
        newChildLink.addEventListener("click", (event) => {
            event.preventDefault();

            // Prevent input triggering autocomplete due to value change
            ...
            if (!showAllSuggestions && lookup.length > 0) {
                ...
                this._config.searchFields.forEach((sf) => {
                    ...
                    if (this._config.fuzzy) {
                        ...
                        found = multiWordMatch(text, lookup);
                    }
                    if (found) {
                        isMatched = true;
                    }
